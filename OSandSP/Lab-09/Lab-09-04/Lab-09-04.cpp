// Lab-09-04.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
size_t count_rows_in_file(LPCWSTR FileName) {
	size_t rows = 0;
...
void waiter(int mlsec) {
	std::this_thread::sleep_for(std::chrono::milliseconds(mlsec));
	stop = 1;
	return;
}

void thread_func(std::wstring& file, LPCWSTR FileName) {
	OVERLAPPED overlp;
		...
		if (wait_status == WAIT_OBJECT_0) {
			...
			for (;;) {
				...
				case FILE_ACTION_MODIFIED:
				{
					...
					swprintf_s(&name_buff[0], (size_t)name_len+1, L"%.*s", name_len, event->FileName);
					if (name_buff.compare(file) == 0) {
						size_t temp = count_rows_in_file(FileName);
						if (row_counter == temp) {
							std::wcout << L"Number of rows in file " << file << L" doesn't change!\n";
						}
						else {
							std::wcout << L"Number of rows in file " << file
								<< L" changed from " << row_counter
								<< L" to " << temp << L"!\n";
							row_counter = temp;
						}
					}
...
BOOL printWatchRowFileTxt(
	...
	std::thread thread(thread_func, std::ref(file), FileName), wait(waiter, mlsec);
	wait.join();
	thread.join();
