...
insRowFileTxt(LPWSTR FileName, LPWSTR str, LONG row)
{
  ...
  HANDLE h_file =
    CreateFileW(FileName, GENERIC_READ, NULL, NULL, OPEN_EXISTING, NULL, NULL);
  ...
  h_file = CreateFileW(
    FileName, GENERIC_WRITE, NULL, NULL, TRUNCATE_EXISTING, NULL, NULL);
  if (h_file == INVALID_HANDLE_VALUE) {
    return FALSE;
  }

  DWORD dw_byteswritten;
  result = WriteFile(h_file,
                     buffer.c_str(),
                     sizeof(wchar_t) * (DWORD)length(&buffer),
                     &dw_byteswritten,
                     NULL);
  CloseHandle(h_file);
...
wmain(int argc, wchar_t** argv)
{
  ...
  for (size_t i = 0; i < sizeof(rows) / sizeof(DWORD); i++) {
    result = insRowFileTxt(argv[1], (LPWSTR)L"Inserted row!", rows[i]);
    if (result) {
