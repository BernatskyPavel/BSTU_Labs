// Lab-09.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
BOOL printFileInfo(LPWSTR FileName) {
	HANDLE h_file = CreateFileW(FileName, NULL, NULL, NULL, OPEN_EXISTING, NULL, NULL);
	if (h_file != INVALID_HANDLE_VALUE) {
		...
		DWORD f_size = (f_data.nFileSizeHigh * (MAXDWORD + 1)) + f_data.nFileSizeLow;
		...
		wprintf_s(L" - File creation date and time: %02i.%02i.%i %02i:%02i:%02i\n", tm_creation.wDay, tm_creation.wMonth, tm_creation.wYear, tm_creation.wHour, tm_creation.wMinute, tm_creation.wSecond);
		wprintf_s(L" - File last modification date and time: %02i.%02i.%i %02i:%02i:%02i\n", tm_modefied.wDay, tm_modefied.wMonth, tm_modefied.wYear, tm_modefied.wHour, tm_modefied.wMinute, tm_modefied.wSecond);
	}
	else {
		return FALSE;
	}
...
BOOL printFileText(LPWSTR FileName) {
	HANDLE h_file = CreateFile(FileName, GENERIC_READ, NULL, NULL, OPEN_EXISTING, NULL, NULL);
