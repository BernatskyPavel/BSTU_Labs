...
{
  if (argc != 2) {
    _tprintf_s(_T("Startup failed. Usage: Lab-08-04x.exe <TIME>. Time must be ")
               _T("in seconds.\n"));
    return 1;
  }
  ...
  DWORD dThreadId = 0;
  HANDLE hThread =
    CreateThread(NULL, 0, ThreadProc, &time, CREATE_SUSPENDED, &dThreadId);
...
ThreadProc(LPVOID arg)
{
  ...
  __int64 qwDueTime =
    -static_cast<__int64>(*reinterpret_cast<LPLONG>(arg)) * _SECOND;
...
void WINAPI
TimerProc(LPVOID lpArgToCompletionRoutine,
