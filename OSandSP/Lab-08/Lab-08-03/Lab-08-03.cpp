...
main()
{
  DWORD dThreadId = 0;
  HANDLE hThread =
    CreateThread(NULL, 0, ThreadProc, NULL, CREATE_SUSPENDED, &dThreadId);
  ...
  while (true) {
    counter += 1;
    if ((static_cast<double>(clock()) - static_cast<double>(start)) /
          CLOCKS_PER_SEC >=
        15.0) {
          ...
          DWORD dwTimerHighValue)
{
  _tprintf_s(_T("Time: %f; Counter value: %u\n"),
             (static_cast<double>(clock()) - static_cast<double>(start)) /
               CLOCKS_PER_SEC,
             counter);
  MessageBeep(0);
}
