...
_tmain(int argc, TCHAR* argv[])
{
  if (argc < 4) {
    ...
    _tprintf_s(_T("Usage: Lab-05-02.exe <Affinity> <Subprocess 1 priority> ")
               _T("<Subprocess 2 priority>\n"));
  ...
  STARTUPINFO
  process_one_startup, process_two_startup;
  ...
  PROCESS_INFORMATION
  process_one_info, process_two_info;
  ...
  WaitForMultipleObjects(
    2,
    new HANDLE[]{ process_one_info.hThread, process_two_info.hThread },
    TRUE,
    INFINITE);
  CloseHandle(process_one_info.hThread);
