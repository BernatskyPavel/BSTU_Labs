            ...
            if (_requestContext.RouteData.Values["api"] != null) {
                switch (_requestContext.RouteData.Values["api"] ?? "") {
                    case "V1":
                        ...
                        break;
                    case "V2": {
                            if (!new string[] { "M01", "M02", Consts.DEFAULT_ACTION }.Contains(action_name)) {
                        ...
                        break;
                    case "V3": {
                            if (_requestContext.RouteData.Values.Keys.Count == 3) {
                                if (action_name != Consts.DEFAULT_ACTION || controller_name != Consts.DEFAULT_CONTROLLER) {
                                    throw new HttpException(404, "NotFound");
                                }
                            }
